...
import { get, getAuthData, post } from "./axios"; // Assuming axios is correctly set up
...
const SubscriptionService = {
  ...
  CreateCustomer: async (data) => {
    const { token, userId } = getAuthData(); // Get token from local storage or elsewhere
    try {
      ...
      if (response.apiCode === 0 && response.data) {
        ...
        console.error("Failed to Subscribe:", response.displayMessage);
        return [];
      }
    } catch (error) {
      console.error(
        "Error Subscribing to the plan:",
        error.response ? error.response.data : error.message
      );
      return [];
    }
